...
import {FormLabel,RadioGroup,FormControlLabel, Radio,Checkbox} from '@material-ui/core';
...
export default function QuizPage({quizDetails, handleRollBack}){
    ...
    const [player, setPlayer] = useState({lastName:"",firstName:"",age:0,gender:"",email:"",cCIName:"",check:"0"});
    ...
    useEffect(() =>{
        ...
        const url = "https://floating-badlands-28885.herokuapp.com/question/quizquestions"
        setLoading(true);

        axios.post(url, bodyParams, header)
        .then(res => {
    ...
    const rows = [
        ...
        createData('Wrong answer given:', (allAnsweredQuestion.length-correctAnsweredQuestion.length)),
    ...
    const paginateHandler = pageNumber => {
        if(pageNumber >= 1 && pageNumber <= Math.ceil(questions.length/questionPerPage)){
    ...
    const handleRollBackClick = () => {
        if(window.location.href === "https://serene-dubinsky-0f4e70.netlify.app/"){
            handleRollBack();
        }else{
            const quizPlayer = {
            ...
            axios.post(url, quizPlayer , headers)
            .then(res => {
                console.log(res.data);
            })
            .catch(err => {
    ...
    const HandleDetailSumbit = (e) => {
        ...
        const data = new FormData(e.target);
        if(data.get('check') === "1"){
            setPlayer({lastName:data.get('lastName'),
        ...
        <div>
            ...
            <div className= "wrapper-instruction">
                <div className="result-box">
                    ...
                    <Link to="/" style={{textDecoration:"none"}}>
                        <Btnrb className='button-tmb' onClick={handleRollBackClick}>Ok</Btnrb>
                    </Link>
                </div>
            </div>:
            <div>
            ...
            <div className="wrapper-qp">
                <div className="content-wrapper-timer">
                    <QuizTimer handleTestSubmit={handleTestSubmit} mins={mins} hours={hours}/>
                </div>
                <Row style={{width:"100%"}}>
                    <Col lg={3} md={12} sm={12} className='content-wrapper-qp'>
                        <Row>
                            <h5 className="quizpage-h5"><b>Answer tracker</b></h5>
                        </Row>
                        <Row>
                    ...
                    <Col  lg={9} md={12} sm={12}>
                        ...
                        <div className='question-area'>
                            <h5 className="quizpage-h5"><b>{quizDetails.title}</b></h5>
                            <ul className="list-group mb-4">
                            {currentQuestions.map((question, index) =>(
                                <Row key={index}>
                                    <form  onSubmit ={(e) => onAnswerSubmit(question.answer,question._id, e)} className="Question-form">
                                        <Col lg={12} md={12} sm={12} className="question-col">
                                            <FormLabel component="legend"><h5><b>Q.{currentPage*questionPerPage+index-questionPerPage+1}</b> {question.question}</h5></FormLabel>
                                        </Col>
                                        <RadioGroup row aria-label="givenAnswer" name="givenAnswer">
                                        ...
                                        <Col lg={12} md={12} sm={12} className="sumbit-button-col">
                                            <Btnrb type="submit" variant="success">
                                                Sumbit Answer
                                            </Btnrb>
                                        </Col>
                                    </form>
                            ...
                            <Btnrb className='button-st' onClick={handleTestSubmit}>Sumbit Test</Btnrb>
            ...
            <div className="wrapper-instruction">
                ...
                {(quizDetails.testType === "private")?
                    <div className="personalInfo">
                        ...
                        <Form className="form-pi" onSubmit ={HandleDetailSumbit} >
                            ...
                            <FormGroup row>
                                <Label for="CCIName" sm={2}>College / Company / institution Name:</Label>
                                <Col sm={10}>
                                    <Input type="text" name="cCIName" id="CCIName" placeholder="Enter College/Company/Institute name."/>
                                </Col>
                            </FormGroup>
                            <FormControlLabel   value="1" name="check" id="exampleCheck"
                                                control={<Checkbox color="primary" />} 
                                                label="I agree, above data is correct and valid as per my knowledge." 
                                                labelPlacement="end"/>
                            <Btnrb type="submit" variant="success" className='button-details'>
                                Save
                            </Btnrb>
                        </Form>
                    </div>:<div></div>
                ...
                <div>
                    <h6><p><b>Ensure you read this guide and instruction from start to end.</b></p></h6>
                    <p><b>1.</b> Each quiz has a limited time. You must finish and sumbit the quiz with in given time limit.</p>
                    <p><b>2.</b> The quiz will be automatically submitted as soon as your time elapses.</p>
                    <p><b>3.</b> After selecting your answer you must submit each question's answer Individially.</p>
                    <p><b>4.</b> Unsumbitted question's answer will not be considered while calculating quiz final score.</p>
                    <p><b>5.</b> You can change your answer multiple time.The final selected answer will be considered while calculating quiz final score.</p>
                    <p><b>6.</b> Answer can be changed even after submitting the answer</p>
                    <p><b>7.</b> Each question contains 4 options and anyone of them would be corect answer</p>
                    <p><b>8.</b> Question numbers in red and green color can be seen at left side of the screen. These represents unanswered and answered question respectively.</p>
                    <p><b>9.</b> Answer can be changed even after submitting the answer.</p>
                    <Row>
                        <Col lg={6} md={6} sm={12}>
                            <Link to="/" style={{textDecoration:"none"}}>
                                <Btnrb className='button-tmb'onClick={handleRollBackClick}>Take me back</Btnrb>
                            </Link>
                        </Col>
                        <Col lg={6} md={6} sm={12}>
